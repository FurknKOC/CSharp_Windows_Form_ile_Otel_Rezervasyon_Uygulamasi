...
namespace projem
{
    public partial class frmHiltonMusteriler : Form
    {
        public frmHiltonMusteriler()
        {
            InitializeComponent();
        }
        ...
        private void frmHiltonMusteriler_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
            SqlCommand cmd = new SqlCommand("select * from HiltonMusteriBilgileri", cnn);
            cmd.Connection.Open();
            ...
            if (rd.HasRows) // Girilen K.Adı ve K.Parola Dahilinde Gelen Data var ise 
            {
                ...
                {
                    dataGridView2.Rows.Add(rd["HiltonMusteriID"].ToString(), rd["TC"].ToString(), rd["Ad"].ToString(), rd["Soyad"].ToString(), rd["BabaAdi"].ToString(), rd["AnneAdi"].ToString(), rd["DogumTarihi"].ToString(), rd["Cinsiyet"].ToString(), rd["MedeniHali"].ToString());
                }
            }
        }
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
                SqlCommand cmd = new SqlCommand("delete from HiltonMusteriBilgileri where HiltonMusteriID=@ID", cnn);
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            Form frm = new frmHiltonGuncelle();
            frm.Show();
        }

        private void button1_Click(object sender, EventArgs e)
        {
            Form frmHiltonMusteriler = new frmHiltonMusteriler();
            frmHiltonMusteriler.Show();
            this.Hide();
        }

        private void MusteriKaydet_Click(object sender, EventArgs e)
        {
            Form frmGit = new frmHiltonRezervasyonIslemleri();
            frmGit.Show();
            this.Hide();
        }
                ...
                while (iRow <= dataGridView2.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (bNewPage)
                        {
                            ...
                            e.Graphics.DrawString("Hilton Otel Müşteriler", new Font(dataGridView2.Font, FontStyle.Bold),
                                    Brushes.Black, e.MarginBounds.Left, e.MarginBounds.Top -
                                    e.Graphics.MeasureString("Hilton Otel Müşteriler", new Font(dataGridView2.Font,
                            ...
                            e.Graphics.DrawString(strDate, new Font(dataGridView2.Font, FontStyle.Bold),
                                    ...
                                    e.Graphics.MeasureString("Hilton Otel Müşteriler", new Font(new Font(dataGridView2.Font,
