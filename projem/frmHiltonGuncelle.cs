...
namespace projem
{
    public partial class frmHiltonGuncelle : Form
    {
        public frmHiltonGuncelle()
        {
            InitializeComponent();
        }

        private void frmHiltonGuncelle_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
            SqlCommand cmd = new SqlCommand("select * from HiltonMusteriBilgileri where HiltonMusteriID=@ID", cnn);
            cmd.Parameters.AddWithValue("@ID", frmHiltonMusteriler.ID);
            cmd.Connection.Open();
            ...
            if (rd.HasRows) // Girilen K.Adı ve K.Parola Dahilinde Gelen Data var ise 
            {
                while (rd.Read()) // reader Okuyabiliyorsa
                {
                    ...
                    txtOdaNumarasi.Text = rd["HiltonOdaID"].ToString();
                }
            }
        }
        ...
        private void MusteriGuncelle_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
                SqlCommand cmd = new SqlCommand("UPDATE HiltonMusteriBilgileri SET TC=@TC,Ad=@Ad,Soyad=@Soyad,BabaAdi=@BabaAdi,AnneAdi=@AnneAdi,DogumTarihi=@DogumTarihi,CepTel=@Ceptel,EvTel=@EvTel,IsTel=@IsTel,Email=@Email,Meslek=@Meslek,Adres=@Adres,HiltonOdaID=@HiltonOdaID where HiltonMusteriID=@ID", cnn);
                cmd.Parameters.AddWithValue("@ID", frmHiltonMusteriler.ID);
                ...
                cmd.Parameters.AddWithValue("@HiltonOdaID", txtOdaNumarasi.Text);
                cmd.Connection.Open();
                cmd.ExecuteNonQuery();
