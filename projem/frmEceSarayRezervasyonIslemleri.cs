...
namespace projem
{
    public partial class frmEceSarayRezervasyonIslemleri : Form
    {
        public frmEceSarayRezervasyonIslemleri()
        {
            InitializeComponent();
        }
        ...
        private void frmEceSarayRezervasyonIslemleri_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
            SqlCommand cmd = new SqlCommand("select * from EceSarayOdaDurum  where EceSarayOdaDurum.EceSarayOdaID=@IDler ", cnn);
            cmd.Parameters.AddWithValue("@IDler", frmEceSarayRezervasyon.odanumarasi);
            cmd.Connection.Open();
        ...
        int fiyat = frmEceSarayRezervasyon.odafiyat;
        double ucret = 0;
        int deger = 0;
