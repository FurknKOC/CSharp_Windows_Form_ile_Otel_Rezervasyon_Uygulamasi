...
namespace projem
{
    public partial class frmEceSarayGuncelle : Form
    {
        public frmEceSarayGuncelle()
        {
            InitializeComponent();
        }

        private void frmEceSarayGuncelle_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
            SqlCommand cmd = new SqlCommand("select * from EceSarayMusteriBilgileri where EceSarayMusteriID=@ID", cnn);
            cmd.Parameters.AddWithValue("@ID", frmEceSarayMusteriler.ID);
            cmd.Connection.Open();
            ...
            if (rd.HasRows) // Girilen K.Adı ve K.Parola Dahilinde Gelen Data var ise 
            {
                while (rd.Read()) // reader Okuyabiliyorsa
                {
                    ...
                    txtOdaNumarasi.Text = rd["EceSarayOdaID"].ToString();
                }
            }
        }
        ...
        private void MusteriGuncelle_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
                SqlCommand cmd = new SqlCommand("UPDATE EceSarayMusteriBilgileri SET TC=@TC,Ad=@Ad,Soyad=@Soyad,BabaAdi=@BabaAdi,AnneAdi=@AnneAdi,DogumTarihi=@DogumTarihi,CepTel=@Ceptel,EvTel=@EvTel,IsTel=@IsTel,Email=@Email,Meslek=@Meslek,Adres=@Adres,EceSarayOdaID=@EceSarayOdaID where EceSarayMusteriID=@ID", cnn);
                cmd.Parameters.AddWithValue("@ID", frmEceSarayMusteriler.ID);
                ...
                cmd.Parameters.AddWithValue("@EceSarayOdaID", txtOdaNumarasi.Text);
                cmd.Connection.Open();
                cmd.ExecuteNonQuery();
