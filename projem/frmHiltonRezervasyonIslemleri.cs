...
namespace projem
{
    public partial class frmHiltonRezervasyonIslemleri : Form
    {
        public frmHiltonRezervasyonIslemleri()
        {
            InitializeComponent();
        }
        ...
        private void frmHiltonRezervasyonIslemleri_Load(object sender, EventArgs e)
        {
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            SqlConnection cnn = new SqlConnection("server =.; Initial Catalog = OtelProje; Integrated Security = SSPI");
            SqlCommand cmd = new SqlCommand("select * from HiltonOdaDurum  where HiltonOdaDurum.HiltonOdaID=@IDler ", cnn);
            cmd.Parameters.AddWithValue("@IDler", frmHiltonRezervasyon.odanumarasi);
            cmd.Connection.Open();
        ...
        int fiyat = frmHiltonRezervasyon.odafiyat;
        double ucret = 0;
        int deger = 0;
